    ...
    private void initialize() {
        ...
        try {
            ...
            if (rs_budget.next() && rs_spending.next() & rs_budgetC.next() && rs_spendingC.next()) {
                ...
                String previousMonth = String.valueOf(df.format(difference));
                String currentMonth = String.valueOf(df.format(differenceC));
        ...
        try {
            ...
            String query = "SELECT categories.name,SUM(spending.spending_amount),categories.budget FROM spending "
                    + "INNER JOIN categories ON categories.categoryID = spending.category_id WHERE spending.user_id = '" + usr.getId() + "' "
                    ...
                    + "GROUP BY categories.budget,categories.name HAVING SUM(spending.spending_amount) >=(categories.budget * 0.90) "
                    + "ORDER BY (SUM(spending.spending_amount)/categories.budget) DESC LIMIT 3";
